...
//
//      void __fastcall TBatchDataInBaseThread::UpdateCaption()
//      {
//        Form1->Caption = "Updated in a thread";
//      }
//---------------------------------------------------------------------------

__fastcall TBatchDataInBaseThread::TBatchDataInBaseThread(bool CreateSuspended,
														  TcxButton* InBTN,
														  TcxButton* BCBTN,
														  TcxButton* EBTN,
														  ...
														  TPanel *DPanel) : TThread(CreateSuspended)
{
	...
	DisplayRichEditView = RichEView;
	ValidPanel = DPanel;
}
//---------------------------------------------------------------------------

void __fastcall TBatchDataInBaseThread::Execute()
{
    ...
    if(true == FileExists(GlobalPath+"\\StroeBatchDataFile.dat"))
	{
		...
		else
		{
			...
			String tmperrfilepath = GlobalPath+"\\WaitForInsertToErrMX.dat";
...
void __fastcall TBatchDataInBaseThread::AnalyzeDataFunc()
{
...
void __fastcall TBatchDataInBaseThread::DisplayDataFunc()
{
/*	String tmphead = PosNumber;
	tmphead += "号POS机数据：";
	DisplayRichEditView->Lines->Add(tmphead);
	DisplayRichEditView->Lines->Add(OrignalDataStr.t_str());
	DisplayRichEditView->Lines->Add("\n");  */
	return;
}
//---------------------------------------------------------------------------

void __fastcall TBatchDataInBaseThread::UpdateDataBaseFunc()
{
...
void __fastcall TBatchDataInBaseThread::SaveDataToFileFunc()
{
