...
//
//      void __fastcall TEMXExportThread::UpdateCaption()
//      {
//        Form1->Caption = "Updated in a thread";
//      }
//---------------------------------------------------------------------------

__fastcall TEMXExportThread::TEMXExportThread(bool CreateSuspended,
...
//---------------------------------------------------------------------------
void __fastcall TEMXExportThread::Execute()
{
	...
	{
		MessageBox(EMXQFrm->Handle, "错误：您的系统中可能没有安装Excel！", "Error in launch Excel!", MB_OK|MB_ICONERROR);
    ...
    Templatepath = path + "\\ExportXLSTemplate\\EMXTemplate.xlt";
    ...
    ST.OlePropertyGet("Cells",2,5).OlePropertySet("Value", ValidEndDatestr.t_str());

    ST.OlePropertyGet("Cells",2,8).OlePropertySet("Value", DateTimeToStr(Now()).t_str());
    ST.OlePropertyGet("Cells",2,11).OlePropertySet("Value", ValidCZYstr.t_str());
    ...
    MessageBox(EMXQFrm->Handle, "数据已完成导出！", "Successfully!", MB_OK|MB_ICONINFORMATION);
